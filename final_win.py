...
#3. untuk membuat tampilan layar ( seperti membuat ukuran DLL )
   def results(self):
       if self.exp.age < 7:
           self.index = 0
...
#4. settingan layar ke 3 ( masih di dalam medtod )
   def initUI(self):
...
#5. Rumus dokter ( menghitung rumus dokter untuk kesehatan jantung )
   #''' sets what the window will look like (label, size, location) '''
   def set_appear(self):
